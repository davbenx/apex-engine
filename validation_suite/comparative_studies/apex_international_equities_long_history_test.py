"""
apex_international_equities_long_history_test.py — Approfondimento
richiesto dall'utente del test precedente
(apex_international_equities_class_test.py, risultato sfumato: MaxDD
migliora davvero con IntlEquities/EFA ma il campione era limitato a 626
settimane/2014-2026 dal vincolo BTC nell'indice comune, dominato dalla
"US exceptionalism" post-2014).

Qui si DISACCOPPIA la classe IntlEquities (e le altre 3 non-crypto) dal
vincolo di storico comune con Crypto — stessa tecnica di raccordo gia'
validata in apex_dashboard_stat_regeneration.py (splice per RENDIMENTO,
non prezzo grezzo): Equities = VFINX (dal 1986-09) -> SPY (dal 1993-01);
Bonds = VUSTX (dal 1986-09) -> IEF (dal 2002-08); Gold = GC=F (dal
2000-08) -> GLD (dal 2004-11); IntlEquities = VGTSX, Vanguard Total
International Stock Index — fondo indice ampio, non growth-tilted come
VWIGX (dal 1996-05) -> EFA (dal 2001-08); Crypto = solo BTC-USD reale
(dal 2014-09), nessun proxy prima (nessun mercato liquido affidabile).

Il walk-forward parte quindi limitato dal piu' tardo tra i proxy delle 4
classi non-crypto: GC=F (oro, 2000-08) — un campione ~14 anni piu' lungo
del precedente (2001-2026 circa contro 2014-2026), niente affatto
dominato in modo cosi' schiacciante dalla "US exceptionalism" recente
(include il decennio 2000-2010, dove i mercati internazionali hanno
storicamente sovraperformato gli USA).

Stesso vincolo di onesta' point-in-time del basket azionario USA (2012+,
select_low_beta_basket) gia' rispettato nel test precedente — invariato
qui, non re-imposto.
"""
...
from apex_stocks_vs_etf_backtest import (
    DATA_DIR, PERIODS_PER_YEAR, load_weekly_sp500, load_pointintime_snapshots,
...
BASKET_SELECTION_FROM_YEAR = 2012
...
SPLICE_SPEC = {
    ...
    "EFA": ("VGTSX_weekly.csv", "EFA_weekly.csv", "2001-08-31"),
...
def precompute_equity_basket(sector_of, weeks, macro_prices, equity_index_ret, stock_prices, stock_rets, snapshots):
    n = len(weeks)
...
def main():
    ...
    macro_prices = {ticker: spliced_price_index(*SPLICE_SPEC[ticker]) for ticker in CLASSES_5.values()}
    macro_rets = {ticker: spliced_return(*SPLICE_SPEC[ticker]) for ticker in CLASSES_5.values()}
    equity_index_ret = macro_rets["SPY"]

    # Limitato dal piu' tardo tra i 4 proxy non-crypto (GC=F/Gold, 2000-08) — Crypto
    # (dal 2014-09) resta a 0% finche' non ha 40 settimane proprie, gestito nativamente.
    common_index = macro_prices["SPY"].index
    for ticker in ("IEF", "GLD", "EFA"):
        common_index = common_index.intersection(macro_prices[ticker].index)
    weeks = list(common_index.sort_values())
    print(f"[*] Campione: {len(weeks)} settimane, {weeks[0].date()} -> {weeks[-1].date()}")

    print("[*] Precalcolo basket azionario low-beta USA (identico in entrambe le varianti, vincolo 2012+ per selezione per-titolo)...")
    equity_return_basket = precompute_equity_basket(sector_of, weeks, macro_prices, equity_index_ret, stock_prices, stock_rets, snapshots)

    print("[*] Baseline (4 classi, attuale)...")
    net_4, w_4 = run_variant(V2_CLASS_TICKER, weeks, macro_prices, macro_rets, equity_return_basket)
    print("[*] 5 classi (+ IntlEquities/EFA, storico esteso)...")
    ...
    for label, net in [("Baseline (4 classi)", n4), ("5 classi (+IntlEquities, esteso)", n5)]:
    ...
    # Sensibilita': sotto-periodo 2001-2014 (escluso dal test precedente) da solo
    pre2014 = common_idx[common_idx < pd.Timestamp("2014-01-01")]
    if len(pre2014) > 52:
        print(f"\n--- Sensibilita': SOLO 2001-2014 ({len(pre2014)} settimane, escluso dal test precedente) ---")
        n4p, n5p = n4.reindex(pre2014), n5.reindex(pre2014)
        for label, net in [("4 classi", n4p), ("5 classi", n5p)]:
            c, s, dd = _cagr(net, PERIODS_PER_YEAR), _sharpe(net, periods_per_year=PERIODS_PER_YEAR), _max_drawdown(net)
            print(f"  {label:<20}CAGR {c*100:>8.2f}%   Sharpe {s:>5.2f}   MaxDD {dd*100:>7.2f}%")

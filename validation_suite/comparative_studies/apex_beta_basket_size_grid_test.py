"""
apex_beta_basket_size_grid_test.py — Con la selezione low-beta decisa
(vedi apex_beta_basket_selection_test.py e i 5 giri di verifica), quale
numero di titoli (V2_EQUITY_TOP_N) funziona meglio? Richiesta diretta
dell'utente ("dobbiamo decidere quante azioni"). Stesso identico harness
di apex_basket_size_grid_test.py (che ha gia' fatto questa domanda per
low-vol, trovando 15 indifferente dal resto della griglia) — l'unica
variabile che cambia e' il criterio di selezione (low-beta, lookback=26
settimane, il valore scelto per la produzione dopo i 5 giri di verifica).

Griglia: {10, 12, 15, 18, 20, 25} titoli, buffer_rank = top_n + 5 (stessa
convenzione del valore attuale).
"""
...
def run_backtest(top_n: int, buffer_rank: int, sector_of: dict):
    ...
    for i, wk in enumerate(weeks):
        ...
        def rebuild_basket():
            ...
            basket = select_low_beta_basket(
                eq_data, spy_data_upto, top_n=top_n, lookback_weeks=LOOKBACK, buffer_rank=buffer_rank,
                prev_tickers=prev_basket_tickers, sector_of=sector_of, max_per_sector=FIXED_MAX_PER_SECTOR,
            )
    ...
    return {
        ...
        "alpha_full": alpha_full, "p_full": p_full,
    }
...
def main():
    ...
    print(f"\nPBO-CSCV su {len(grid)} taglie di basket (low-beta): {pbo*100:.1f}%")
    ...
    print(f"Migliore per Sharpe netto: top_n={best_n} (Sharpe {results[best_n]['sharpe_netto']:.2f} vs "
          f"{results[15]['sharpe_netto']:.2f} di top_n=15)")
    print(f"  DSR (n_trials={len(grid)}, conteggio reale): {dsr:.4f}")
    print(f"  CI 90% Sharpe (block bootstrap): [{lo:.2f}, {hi:.2f}]")

    diff = (results[best_n]["net"] - results[15]["net"]).dropna()
    mean_diff = diff.mean() * PERIODS_PER_YEAR * 100
    lo_d, hi_d = block_bootstrap_ci(diff.values, lambda r: pd.Series(r).mean() * PERIODS_PER_YEAR * 100,
                                     block_size=12, ci=0.90, seed=42)
    print(f"\nConfronto accoppiato diretto (top_n={best_n} meno top_n=15):")
    print(f"  Overperformance media: {mean_diff:+.2f}pp/anno, CI 90% [{lo_d:+.2f}, {hi_d:+.2f}]pp/anno "

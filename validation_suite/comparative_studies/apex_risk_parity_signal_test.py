"""
apex_risk_parity_signal_test.py — Teoria accademica #3 da sondare: Apex V2
assegna lo STESSO base_weight_per_class nozionale a ogni classe attiva
(es. Crypto e Bonds ricevono entrambi 50% se entrambi attivi), poi scala
SOLO il portafoglio aggregato al vol-target — non pesa le classi per la
loro volatilita' INDIVIDUALE. Questo significa che, a peso nozionale
uguale, Crypto (molto piu' volatile di Bonds) contribuisce MOLTO piu'
rischio al portafoglio di quanto Bonds contribuisca — non e' risk parity.

La letteratura di risk parity (Qian 2005; Maillard-Roncalli-Teiletche 2010,
"On the Properties of Equally-Weighted Risk Contribution Portfolios") mostra
che pesare per il CONTRIBUTO AL RISCHIO invece che per il capitale nozionale
tende a migliorare lo Sharpe risk-adjusted, specialmente quando le classi
hanno volatilita' molto diverse (qui: Bonds ~5-8% annua contro Crypto
~50-70% annua).

Costruzione testata (naive risk parity, pesi inversamente proporzionali
alla volatilita' realizzata — NON la vera ERC con covarianza, che
richiederebbe stime di correlazione instabili su finestre corte,
semplificazione dichiarata, standard nella pratica quando le classi sono
poco correlate come qui): stessa identica logica di ingresso/uscita
(isteresi + conferma multi-timeframe) della produzione — cambia SOLO come
si distribuisce il peso nozionale TRA le classi attive, mantenendo la
stessa esposizione lorda pre-vol-target di prima (n_classi_attive x
base_weight_per_class, solo redistribuita per 1/volatilita' invece che
in parti uguali).
...
def compute_signal_risk_parity(
    ...
    """Identica a compute_v2_macro_signal per timing di ingresso/uscita — l'UNICA
    differenza e' come si distribuisce il peso nozionale TRA le classi attive:
    inversamente proporzionale alla volatilita' realizzata (naive risk parity)
    invece che in parti uguali, mantenendo la stessa esposizione lorda totale
    pre-vol-target (n_attive x base_weight_per_class)."""
    ...
    for cls, ticker in V2_CLASS_TICKER.items():
        ...
        debug[cls] = {"distanza_pct": round(dist * 100, 2), "attivo": is_active}
    ...
    if n_active > 0:
        active_with_vol = [c for c in active_classes if c in vols]
        if active_with_vol:
            inv_vol_sum = sum(1.0 / vols[c] for c in active_with_vol)
            for cls in active_with_vol:
                base_weight[cls] = base_weight_per_class * n_active * (1.0 / vols[cls]) / inv_vol_sum
            for cls in active_classes:
                if cls not in vols:
                    base_weight[cls] = base_weight_per_class  # fallback: vol non disponibile
        else:
            for cls in active_classes:
...
def main():
    ...
    candidate = run_backtest(compute_signal_risk_parity, sector_of)
    calmar_c = candidate["cagr_netto"] / abs(candidate["maxdd_netto"])
    print(f"{'Risk parity (peso inv. proporzionale a vol)':<45}{candidate['cagr_netto']*100:>11.2f}%{candidate['sharpe_netto']:>14.2f}"
          f"{candidate['maxdd_netto']*100:>12.2f}%{calmar_c:>9.2f}{candidate['avg_gross_exposure']*100:>12.1f}%")

    print("\nEsposizione media per classe (quando attiva) — baseline vs risk parity:")
    for cls in ["Equity", "Bonds", "Gold", "Crypto"]:
        ...
        print(f"  {cls:<8} baseline: {ab.mean()*100:>5.1f}% (n={len(ab)})   "
              f"risk parity: {ac.mean()*100:>5.1f}% (n={len(ac)})")
    ...
    print(f"\nConfronto accoppiato diretto (risk parity meno baseline nozionale uguale):")
    print(f"  Overperformance media: {mean_diff:+.2f}pp/anno, CI 90% [{lo:+.2f}, {hi:+.2f}]pp/anno "
          f"({'ESCLUDE' if lo * hi > 0 else 'INCLUDE'} lo zero)")
    print(f"  Settimane in cui risk parity ha fatto meglio: {n_better}/{len(diff)} ({n_better/len(diff)*100:.0f}%)")

    dsr = deflated_sharpe_ratio(candidate["sharpe_netto"], n_trials=2, n_obs=len(candidate["net"]))
    print(f"  DSR risk parity (n_trials=2): {dsr:.4f}")
